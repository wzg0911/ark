...
## 案例四：合法空输出被误判为故障——备胎静默上位

**问题来源：** langchain-ai/langchain#38892（2026-07-16 提交，bug/core 标签，13 评论）

**用户痛点：**
`RunnableWithFallbacks.stream()/astream()` 用裸 `next(stream)` 窥视首 chunk 判断成败。
空流是完全合法的终态（LLM 空回复、过滤步骤、审核拦截），但 `StopIteration` 是
Exception 子类，被默认 `exceptions_to_handle=(Exception,)` 捕获当成故障：
**配 fallback → 备胎输出无声替换合法空输出（零信号）；不配 → 误导性
`RuntimeError: generator raised StopIteration`。**

**根因分析：**
```
裸 next(stream) 窥视首 chunk 判断"成功"
→ 空流 StopIteration 混入 except Exception 分支
→ "流为空"与"流抛错"两个正交问题折叠成一个失败分支
→ fallback 触发 / PEP 479 转 RuntimeError
→ 控制流信号被当成故障信号
```

**社区侧信号（放大传播价值）：**
9 个修复 PR（sentinel 窥视方向已收敛共识）全部被 require-issue-link bot 自动关闭，
上游修复被流程卡死——**用户侧防线（ARK）是当前唯一可落地的方案**。

**ARK Trust 修复方案：**
- `OutputValidator` — "输出必须来自主链路"不变式，备胎无声上位 1 秒暴露
- OTel Bridge — `ark.fallback.activated` 留痕事件，替换从不可知变成可审计报表
- 上游 sentinel 修复（`next(stream, _MISSING)`）合入前，输出校验是唯一防线

**报告链接：** `docs/reports/ark-report-38892-20260724.html`
